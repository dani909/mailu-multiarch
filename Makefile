...
admin: source
	buildctl build $(FRONTEND) --local dockerfile=build/core/admin --local \
	 context=build/core/admin --output type=image,name=$(REPO)/$(PREFIX)admin:$(VERSION) \
...
dovecot: source
	...
	buildctl build $(FRONTEND) --local dockerfile=build/core/dovecot --local \
	 context=build/core/dovecot --output type=image,name=$(REPO)/$(PREFIX)dovecot:$(VERSION) \
...
nginx: source
	buildctl build $(FRONTEND) --local dockerfile=build/core/nginx --local \
	 context=build/core/nginx --output type=image,name=$(REPO)/$(PREFIX)nginx:$(VERSION) \
...
none: source
	buildctl build $(FRONTEND) --local dockerfile=build/core/none --local \
	 context=build/core/none --output type=image,name=$(REPO)/$(PREFIX)none:$(VERSION) \
...
postfix: source
	buildctl build $(FRONTEND) --local dockerfile=build/core/postfix --local \
	 context=build/core/postfix --output type=image,name=$(REPO)/$(PREFIX)postfix:$(VERSION) \
...
clamav: source
	buildctl build $(FRONTEND) --local dockerfile=build/optional/clamav --local \
	 context=build/optional/clamav --output type=image,name=$(REPO)/$(PREFIX)clamav:$(VERSION) \
...
postgresql: source
	buildctl build $(FRONTEND) --local dockerfile=build/optional/postgresql --local \
	 context=build/optional/postgresql --output type=image,name=$(REPO)/$(PREFIX)postgresql:$(VERSION) \
...
radicale: source
	buildctl build $(FRONTEND) --local dockerfile=build/optional/radicale --local \
	 context=build/optional/radicale --output type=image,name=$(REPO)/$(PREFIX)radicale:$(VERSION) \
...
traefik-certdumper: source
	buildctl build $(FRONTEND) --local dockerfile=build/optional/traefik-certdumper --local \
	 context=build/optional/traefik-certdumper --output type=image,name=$(REPO)/$(PREFIX)traefik-certdumper:$(VERSION) \
...
fetchmail: source
	buildctl build $(FRONTEND) --local dockerfile=build/services/fetchmail --local \
	 context=build/services/fetchmail --output type=image,name=$(REPO)/$(PREFIX)fetchmail:$(VERSION) \
...
rspamd: source
	...
	buildctl build $(FRONTEND) --local dockerfile=build/services/rspamd --local \
	 context=build/services/rspamd --output type=image,name=$(REPO)/$(PREFIX)rspamd:$(VERSION) \
...
unbound: source
	buildctl build $(FRONTEND) --local dockerfile=build/services/unbound --local \
	 context=build/services/unbound --output type=image,name=$(REPO)/$(PREFIX)unbound:$(VERSION) \
...
rainloop: source
	buildctl build $(FRONTEND) --local dockerfile=build/webmails/rainloop --local \
	 context=build/webmails/rainloop --output type=image,name=$(REPO)/$(PREFIX)rainloop:$(VERSION) \
...
roundcube: source
	buildctl build $(FRONTEND) --local dockerfile=build/webmails/roundcube --local \
	 context=build/webmails/roundcube --output type=image,name=$(REPO)/$(PREFIX)roundcube:$(VERSION) \
